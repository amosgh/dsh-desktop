...
Before submitting a change, run `npm run typecheck`, `npm test`, and the relevant smoke tests. UI changes must be checked in light/dark appearance, reduced motion, keyboard-only use, and at the 960 px minimum width. Security-sensitive changes require tests for invalid IPC input, path escape, duplicate approval resolution, and secret redaction as applicable.
