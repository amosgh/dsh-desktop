...
HTTP/HTTPS links opened in the right-side in-app browser are requested directly from that site in an isolated, in-memory browser session. The browser denies permission requests and downloads, but the destination still receives ordinary network metadata such as the IP address and request headers. Right-side Markdown preview itself is local and does not automatically load remote images or other embedded resources.
